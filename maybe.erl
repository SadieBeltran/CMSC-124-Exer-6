...
pong(User2, Ping_Pid) ->
    ...
    if
        ...
        Message == "bye\n" -> pongLeave(Ping_Pid);
        Message /= "bye\n" -> 
            Ping_Pid ! {User2, Message},    % sends message to ping_pid from User2
            pong(User2, Ping_Pid)
    end.
...
init_chat3(Pong_Node) ->
    User3 = string:strip(io:get_line('Enter your name: '), right, $\n),     %% removes '\n' in name (Reference: https://stackoverflow.com/a/18573368)
    spawn(maybe, pang, [User3, Pong_Node]).

% sending Pid to pong user
pang(User3, Pong_Node, Ping_Pid) ->
    {pong, Pong_Node} ! self(),  
    Ping_Pid ! self(),  
    pang1(User3, Pong_Node).
